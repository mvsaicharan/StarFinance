  ...
  onSubmit(): void {
    ...
    if (!this.isKycVerified) { 
        ...
        if (this.kycForm.valid && this.kycForm.get('declaration')?.value) {
            ...
            alert('✅ Application for KYC is submitted. Please wait a few seconds for verification...');
            const formData = this.kycForm.value;

            this.authService.submitKyc(formData).subscribe({
                next: (response: any) => {
                    // ALERT: Submission Success (Initial KYC)
                    alert(`✅ KYC verified successfully! Your KN Number is: ${response.knNumber}. Redirecting to dashboard...`);
                ...
                error: (err: HttpErrorResponse) => {
                    alert(`❌ KYC verification failed: ${err.error?.message || 'Server error'}`);
            ...
            alert('⚠️ Please complete all required fields and agree to the declaration.');
            this.submitting = false;
        }
    } else {
        ...
        if (this.kycForm.get('mobileNumber')?.valid && 
            ...
            // ALERT: Update Start
            alert('✅ Submitting updated profile details...');
            // Use getRawValue() to include disabled (unchanged) core fields
            const formData = this.kycForm.getRawValue(); 

            this.authService.submitKyc(formData).subscribe({
                next: (response: any) => {
                    // 💡 ALERT FIX: Profile updated successfully!
                    alert('✅ Profile updated successfully!');
                    this.submitting = false;
                    this.loadCustomerProfileSafely(); // NEW: Use safe reload
                },
                error: (err: HttpErrorResponse) => {
                    alert(`❌ Profile update failed: ${err.error?.message || 'Server error'}`);
                    this.submitting = false;
                }
            });
        } else {
            this.kycForm.markAllAsTouched();
            alert('⚠️ Please ensure all editable fields are valid.');
            this.submitting = false;
        }
    }
  }
